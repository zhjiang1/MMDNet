...
class MMDNet(nn.Module):
    def __init__(
        ...
        self.out_steps = out_steps#输出时间步
        self.num_layers = num_layers#层数
        self.cheb_k = cheb_k#2
        self.ycov_dim = ycov_dim#2
        self.node_embedding_dim = node_embedding_dim#节点嵌入
        self.st_embedding_dim = st_embedding_dim#时空嵌入
        self.tf_decay_steps = tf_decay_steps
        self.use_teacher_forcing = use_teacher_forcing#True

        self.extractor_s = ST_relation_extractor(
            input_dim,
            hidden_dim,
            cheb_k,
            num_layers,
        ...
        self.extractor_t = ST_relation_extractor(
            input_dim,
            hidden_dim,
            cheb_k,
            num_layers,
        ...
        self.cross_extractor = Cross_relation_extractor(
            64,
            hidden_dim,
            cheb_k,
            num_layers,
    ...
    def forward(self,x,y_cov,labels=None, batches_seen=None):
        ...
        h_s_1,h_s_2,h_s_3= self.extractor_s(x1,x2,x3,x1_time_adj,x2_time_adj,x3_time_adj,self.node_embedding1,
                       self.node_embedding2,self.node_embedding3,self.mode_embedding_s)
        h_t_1,h_t_2,h_t_3= self.extractor_t(x1,x2,x3,x1_time_adj,x2_time_adj,x3_time_adj,x1_time_embedding,
        ...
        for t in range(self.out_steps):
            h_de, ht_list = self.cross_extractor(
                h_last,
                ht_list,
...
class ST_relation_extractor(nn.Module):
    def __init__(
        self,
        input_dim,#3
        output_dim,#64
        ...
        self.cells = nn.ModuleList(
            [Mode_GCRU(input_dim, output_dim, cheb_k, embed_dim, meta_axis)]
            + [
               Mode_GCRU(output_dim, output_dim, cheb_k, embed_dim, meta_axis)
    ...
    def forward(self, x1,x2,x3,support1,support2,support3,embeddings1,embeddings2,embeddings3,mode_embedding):
        ...
        for cell in self.cells:
            state1 = cell.init_hidden_state(batch_size,num_nodes1).to(x1.device)
            state2 = cell.init_hidden_state(batch_size,num_nodes2).to(x1.device)
            state3 = cell.init_hidden_state(batch_size,num_nodes3).to(x1.device)
...
class Cross_relation_extractor(nn.Module):
    ...
    def __init__(
        ...
        self.cells = nn.ModuleList(
            [Cross_Mode_GCRU(input_dim, output_dim, cheb_k, embed_dim, meta_axis)]
            + [
                Cross_Mode_GCRU(output_dim, output_dim, cheb_k, embed_dim, meta_axis)
                for _ in range(1, num_layers)
            ]
        )
...
class Cross_Mode_GCRU(nn.Module):
    ...
    def __init__(
        ...
        self.gate = Cross_Mode_GCN(
            input_dim + self.hidden_dim, 2 * output_dim, cheb_k, embed_dim, meta_axis
        )
        self.update = Cross_Mode_GCN(
            input_dim + self.hidden_dim, output_dim, cheb_k, embed_dim, meta_axis
        )
...
class Cross_Mode_GCN(nn.Module):
